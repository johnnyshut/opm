...
Процедура СоздатьСкриптЗапуска(Знач ИмяСкриптаЗапуска, Знач ПутьФайлаПриложения, Знач Каталог) Экспорт
	...
	Если ЭтоWindows Тогда
		...
		ПутьФайлаДляBat = ПолучитьПутьФайлаДляBat(ПутьФайлаПриложения);
		ФайлЗапуска.ЗаписатьСтроку("@call oscript """ + ПутьФайлаДляBat + """ %*");
...
Функция ПолучитьПутьФайлаДляBat(Знач ПутьФайлаПриложения)
	Если мРежимУстановкиПакетов <> РежимУстановкиПакетов.Локально Тогда
		Возврат ПутьФайлаПриложения;
	КонецЕсли;

	ПутьВнутриЛокальнойУстановки = ПолучитьПутьВнутриЛокальнойУстановки(ПутьФайлаПриложения);
	Если ПустаяСтрока(ПутьВнутриЛокальнойУстановки) Тогда
		Возврат ПутьФайлаПриложения;
	КонецЕсли;

	Возврат "%~dp0..\" + ПутьВнутриЛокальнойУстановки;
КонецФункции

Функция ПолучитьПутьВнутриЛокальнойУстановки(Знач ПутьФайлаПриложения)
	МаркерЛокальнойУстановки = "\" + КонстантыOpm.ЛокальныйКаталогУстановкиПакетов + "\";
	ПозицияМаркера = СтрНайти(НРег(ПутьФайлаПриложения), НРег(МаркерЛокальнойУстановки));
	Если ПозицияМаркера = 0 Тогда
		Возврат "";
	КонецЕсли;

	Возврат Сред(ПутьФайлаПриложения, ПозицияМаркера + СтрДлина(МаркерЛокальнойУстановки));
КонецФункции
